(function ($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.rulemakingIndexTableCombineAndSort = {
    attach: function () {
      // Combine the table output from two views into one table and sort them
      // View: Taxonomy term (Rulemaking Index)
      // View: Taxonomy term (Rulemaking Index) Media
      $(document).ready(function () {
        $('.rulemaking-table-nodes').once().each(function () {
          let rowsArrray = [];

          // Get the thead
          let rulemakingTableHead;
          $('.rulemaking-table-nodes thead').once().each(function () {
            let $this = $(this);
            rulemakingTableHead = $this.prop('outerHTML');
          });

          // get Data
          $('.rulemaking-row').once().each(function (index) {
            let $this = $(this);
            $this.addClass('rulemaking-row__processed').removeClass('rulemaking-row');
            let obj = {
              datetime: $this.find('time').attr('datetime'),
              index: index,
              row: $this.prop('outerHTML')
            };
            rowsArrray.push(obj);
          });
          ...
          if (rowsArrray.length !== 0) {
            ...
            // Print Markup
            $('.processed-table').append('<table class="list processed-table">' + rulemakingTableHead + rowsMarkup + '</table>');
